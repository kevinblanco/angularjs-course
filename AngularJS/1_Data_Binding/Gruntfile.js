...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    notify: {
      pages: {
        options: {
          title: '<%= pkg.name %>',
          message: 'HTML modifications - Done!'
        }
      },
      scripts: {
        options: {
          title: '<%= pkg.name %>',
          message: 'JS Modifications - Minifying Done!'
        }
      },
      styles: {
        options: {
          title: '<%= pkg.name %>',
          message: 'SASS Modifications - Recompilation Done!'
        }
      }
    },
  ...
  // Default task.
  grunt.registerTask('run', ['clean', 'uglify:vendor', 'uglify:pop', 'copy:html', 'sass:local', 'livereload-start', 'connect:server', 'regarde']);
